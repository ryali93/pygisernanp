...
# path_salida = r'E:\sernanp\proyectos\monitoreo\sentinel\test'
...
def get_reliability(feature, folder, list_ids):
    id_imgs = ["080W_10S_070W_00N", "080W_20S_070W_10S", "070W_20S_060W_10S"]
    feature_pt = arcpy.FeatureToPoint_management(feature, os.path.join(SCRATCH, "feature_pt"), "INSIDE")
    arcpy.gp.ExtractMultiValuesToPoints_sa(feature_pt,
                                           '{} conf1;{} conf2;{} conf3'.format(
                                               os.path.join(folder, "alert080W_20S_070W_10S.tif"),
                                               os.path.join(folder, "alert080W_20S_070W_10S.tif"),
                                               os.path.join(folder, "alert080W_20S_070W_10S.tif")
                                               ),
                                           'NONE')
    field_conf = "conf"
    if field_conf not in [x.name for x in arcpy.ListFields(feature_pt)]:
        arcpy.AddField_management(feature_pt, field_conf, "SHORT")

    arcpy.CalculateField_management(feature_pt, 'conf', 'fff(!conf1!, !conf2! , !conf3! )  ', 'PYTHON_9.3',
                                    """
                                    def fff(x,y,z):
                                        if x != None and x != 0:
                                            return x
    ...
    arcpy.CalculateField_management(feature_pt, 'md_conf', 'fff( !conf! ) ', 'PYTHON_9.3',
                                    """
                                    def fff(x):
                                        if x in [0,1,2]:
                                            return 2
...
def process():
    ...
    arcpy.AddMessage("\nTermina descarga")
    print("Ruta de imagenes: {}" .format(temp_folder))
