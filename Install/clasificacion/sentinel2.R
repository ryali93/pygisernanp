...
SentinelLima = Sentinel$filterBounds(LimaBnd)$
...
# SentinelLimapreprocessed_1 = SentinelLima$map(maskS2clouds)
# SentinelLimapreprocessed_2 = SentinelLimapreprocessed_1$map(scaling)
# SentinelLimapreprocessed_3 = SentinelLimapreprocessed_2$map(calcVIsSen)
# ee_print(SentinelLimapreprocessed)

# Convertir la colección a una imagen usando estadísticas basadas en pixel y recortar la imagén a la región de Madre de Dios
medianSEN = SentinelLimapreprocessed$median()$clip(LimaBnd)
...
Map$addLayer(classificacionCART$clip(geometry), list(min=0, max=8, palette=paletaColor), 'classificación CART')
