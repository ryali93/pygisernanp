...
# FUNCIONES
# ---------------------------------------------------------------------------------------------------------------------
def download_atd_radd(path, id_img):
    url_download = u"https://data-api.globalforestwatch.org/dataset/wur_radd_alerts/latest/download/geotiff"
    params = {
        "grid":"10/100000",
        "tile_id": id_img,
    ...
    response = requests.get(url_download, params)
    data = response.content
    with open(path, 'wb') as s:
        s.write(data)
    print("\timg radd {} descargada".format(id_img))

# def filter_atd_max(id_img):
#     arcpy.gp.Con_sa('{}.tif'.format(id_img), '00N_080W_f.tif', r'C:\Users\ryali93\Documents\ArcGIS\Default.gdb\Con_tif1', '#', '"Value">30000')
...
def extract_date(date):
    ...
    date_list_value = {x: (base + datetime.timedelta(days=x)).strftime("%d/%m/%Y") for x in
                       range(fecha_ini, fecha_ini + numdays)}
    treshold = [l[0] for l in date_list_value.items() if l[1] == date][0]
    return treshold, date_list


def raster_to_pol(image):
    polygon = arcpy.RasterToPolygon_conversion(image, "in_memory\\polygon", 'NO_SIMPLIFY', 'Value', 'MULTIPLE_OUTER_PART')
...
def filter_atd(image, treshold):
    filtered_image = arcpy.gp.ExtractByAttributes_sa(image, '"Value" > {}'.format(treshold),
                                                     os.path.join(SCRATCH_FOLDER, "f_image.tif"))
...
def update_field_tipobosque(fc, ref_fc, ref_fc_2):
    ...
    arcpy.SelectLayerByLocation_management(fc_mfl, 'INTERSECT', defor_acum, '0 Kilometers', 'NEW_SELECTION',
                                           'NOT_INVERT')
    arcpy.SelectLayerByLocation_management(fc_mfl, 'INTERSECT', defor_antes_2000, '0 Kilometers', 'ADD_TO_SELECTION',
                                           'NOT_INVERT')
...
def process_n():
    temp_folder = r'E:\sernanp\proyectos\monitoreo\radd'
    path_scene = os.path.join(temp_folder, "alert_radd.tif")
    path_out = os.path.join(temp_folder, "alert_radd.shp")
    # download_atd_radd(path_scene, id_img="00N_080W")

    scene_clip = clip_to_anp(path_scene)
    print(path_scene)
    scene_filter = filter_atd(scene_clip, 30000)
    arcpy.AddMessage("\tTermino el filtro de alta confiabilidad")
    pol = raster_to_pol(scene_filter)
    arcpy.AddMessage("\tSe convirtio a poligono")
    arcpy.CopyFeatures_management(pol, path_out)
