...
anp_dic = {
    ...
    "PN14": "035ad9e7-02f0-0f3e-12d3-8464df17ca04",     # 10: PN Sierra del Divisor
    ...
    # "PN08": "b3e57707-1a52-c4ed-01ef-c0a10ba6bbf9",     # 24: PN Pacaya Samiria
    ...
    # "SN06": "5cda4026-c55e-c2d5-3bd5-ea10baaced0",      # 29: SN Megantoni
    # "SN09": "0d552ec6-e312-ffd0-41d0-558f3f9dcaf",      # 30: SN Cordillera Colan
    ...
    # "SH03": "ad153b1a-43c2-d66a-4ede-d33aefbd918a"     # 38: SH MachuPicchu
}
...
def create_tmp_feature(sr):
    fc = arcpy.CreateFeatureclass_management(SCRATCH_FOLDER, "tmp.shp", "POLYGON", "#", "#", "#", arcpy.SpatialReference(sr))
    return fc

def get_data(anp, anp_codi, folder):
...
def create_polygon(df):
    ...
    df = df[1:]
    # df = df[df['gfw_integrated_alerts__confidence'] == "highest"]
    df = df.dropna()

    tmp = create_tmp_feature(32718)
    arcpy.AddField_management(tmp, "md_fecimg", "DATE")
    # arcpy.AddField_management(tmp, "anp_codi", "TEXT", "#", "#", 5)
    # print(df.shape[0])
    ...
    for i in range(df.shape[0]):
        ...
        cursor.insertRow([polygon_proj, md_fecimg])
    del cursor
...
def process():
    ...
    print("Se ha creado una carpeta temporal en la siguiente ruta: {}".format(temp_folder))

    print("\tPaso 1")
    for anp_codi in anp_dic.keys():
        # try:
        print("Ejecutando codigo para {}".format(anp_codi))
        # 1. Obtener la data de GFW a traves del API
        get_data(anp_dic[anp_codi], anp_codi, temp_folder)
    ...
    print("\tPaso 3")
    total = generar_objeto(total_tmp)
    # 4. Actualizar campos
    # print("\tPaso 4")
    # actualizar_campos(total, anp_codi)
    # print("\n")
    # except:
    #     print("Hubo un error")
